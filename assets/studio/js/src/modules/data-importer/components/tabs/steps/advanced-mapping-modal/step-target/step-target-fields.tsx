...
import { type DynamicTypeDataTargetRegistry } from '../../../../../dynamic-types/data-target/dynamic-type-data-target-registry'
import { type DynamicTypeDataTargetAbstract } from '../../../../../dynamic-types/data-target/common/dynamic-type-data-target-abstract'
...
export const StepTargetFields = ({
  ...
  const selectedTarget: DynamicTypeDataTargetAbstract | undefined = useMemo(
    () => targetRegistry.getDynamicType(dataTarget?.type ?? '', false),
    [targetRegistry, dataTarget?.type]
  )
  const targetOptions = useMemo(
    () => targetRegistry.getDynamicTypes().map(({ id, label }) => ({ value: id, label: t(label) })),
    [targetRegistry]
  )

  function onTargetChange (type: string): void {
    const nextTarget = targetRegistry.getDynamicType(type ?? '', false)
