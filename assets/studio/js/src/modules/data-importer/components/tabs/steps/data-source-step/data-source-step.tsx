...
export const DataSourceStep = ({ configName }: DataSourceStepProps): React.JSX.Element => {
    ...
    return (
        <FieldWidthProvider fieldWidthValues={{ medium: 900 }}>
            ...
            <DataImporterPanel>
                <Form.Item label={t('data-importer.data-source.type-label')} name={['loaderConfig', 'type']} required>
                    <Select filterOption={filterByLabel} options={loaderTypes} showSearch />
                </Form.Item>
                ...
                {loaderRegistry.getAllTypes().map((loaderType) => (
                    <Form.Conditional
                        ...
                        key={loaderType.id}
                    >
                        <DataImporterPanel theme="fieldset" title={t(loaderType.label)}>
            ...
            <DataImporterPanel title={t('data-importer.file-format.title')}>
                <Form.Item label={t('data-importer.file-format.title')} name={['interpreterConfig', 'type']} required>
                    <Select filterOption={filterByLabel} options={interpreterTypes} showSearch />
                </Form.Item>
                ...
                {interpreterRegistry.getAllTypes().map((interpreterType) => (
                    ...
                    >
                        <DataImporterPanel border theme="fieldset" title={t(interpreterType.label)}>
