...
/**
 * Shared plumbing for the Data Importer MCP tools: the explicit in-tool permission check and
 * the JSON envelopes.
 *
 * MCP tools bypass the #[IsGranted] / kernel.exception pipeline the Studio controllers rely
 * on, so every tool has to gate itself against the permission those controllers require.
 *
 * @internal
 */
trait DataImporterToolHelper
{
    ...
    /**
     * Null when the current user may configure imports, otherwise a ready-to-return error.
     * Never throws: an unauthenticated user, an unresolvable one and a failing permission
     * lookup are all a denial, so this can run before the caller opens its own try block.
     */
    ...
    /**
     * Terminal catch of every tool here: the handler decides disclosure, this decides
     * presentation — only an argument error comes back verbatim, everything else becomes a
     * generic sentence plus a correlation id, with the exception logged in full.
     *
    ...
    /**
     * Compact on purpose: the payload is read by a model. JSON_INVALID_UTF8_SUBSTITUTE
     * guarantees a string even when a label carries invalid UTF-8, which would otherwise
     * collapse the envelope to an empty result that reads as success.
     *
     * @param array<string, mixed> $payload
     */
